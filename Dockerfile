...
ARG DB_DIR=/app/db
ARG ADMIN_HASH_FILE=/run/secrets/tiddlypwa_admin_hash
...
if [ ! -f "$ADMIN_HASH_FILE" ] || [ ! -s "$ADMIN_HASH_FILE" ]; then
    echo "Error: admin hash file does not exist or is empty. Generate admin hash and salt by running 'podman run -it --rm tiddlypwa-sync-server /app/hash-admin-password.sh', put output in file and mount at path $ADMIN_HASH_FILE"
    exit 1
fi
